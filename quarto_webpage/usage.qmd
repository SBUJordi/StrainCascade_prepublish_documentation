...
As with the initial installation, you first need to navigate to the StrainCascade directory:

``` bash
cd /path/to/your/StrainCascade
```

Then you can run the following command to generate the batch submission:

``` bash
.scritps/SBATCH_scripts/StrainCascade_high_throughput_SBATCH_sequencing_files.sh /path/to/input/directory your_partition [your.email@example.com] [/path/to/file_list.txt]
```
...
-   By default the script will generate a submission script for each file in the input directory and submit them to the SLURM scheduler. Optionally, you can provide a text file limiting execution to specific samples files in the input directory.
-   By default the script will execute a standard run of StainCascade for each of the samples as SBATCH job: `straincascade -i "$input_file" -r main -t 32` (using 96GB of RAM). If you are experienced with StrainCascade and using SBATCH and want to customize the execution, you can modify the script to your needs.
...
``` bash
.scritps/SBATCH_scripts/StrainCascade_high_throughput_SBATCH_assembly_files.sh /path/to/input/directory your_partition [your.email@example.com] [/path/to/file_list.txt]
```
